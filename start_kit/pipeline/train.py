...
def _precompute_val_item_embeddings(
    encoder,
    val_triples: list[tuple[int, int, float]],
    idx2item: dict[int, str],
    device: str,
) -> dict[int, torch.Tensor]:
    """Encode held-out item texts once; same strings/format as ``predict()``."""
    val_ids = sorted({t[1] for t in val_triples})
    texts = [idx2item[i] for i in val_ids]
    print(
        f"Live-val path: encoding {len(texts)} unique held-out items with {ENCODER_NAME} "
        f"(matches competition predict())..."
    )
    with torch.no_grad():
        embs = encoder.encode(
            texts,
            batch_size=VAL_ENCODE_BATCH_SIZE,
            show_progress_bar=False,
            convert_to_tensor=True,
        )
    if not isinstance(embs, torch.Tensor):
        embs = torch.tensor(embs, dtype=torch.float32)
    embs = embs.float().to(device)
    return {vid: embs[j] for j, vid in enumerate(val_ids)}


def _predict_live_batch(
    model,
    s_idx: torch.Tensor,
    i_idx: torch.Tensor,
    val_emb_by_id: dict[int, torch.Tensor],
) -> torch.Tensor:
    """P(correct) from fresh item embeddings + subject ability (predict path)."""
    theta = model.ability[s_idx]
    item_embs = torch.stack([val_emb_by_id[int(i)] for i in i_idx.detach().cpu().tolist()])
    params = model.item_net(item_embs)
    b = params[:, 0]
    a = torch.exp(params[:, 1]) if model.pl >= 2 else None
    c = torch.sigmoid(params[:, 2]) if model.pl == 3 else None
    return model._irt_probability(theta, b, discrimination=a, guessing=c)
...
def _run_epoch_val_live(
    ...
    """Validation BCE using live MPNet item vectors (same path as ``predict()``)."""
    model.eval()
    total_loss = 0.0
    n_batches = 0
    ...
    with torch.no_grad():
        for start in range(0, len(triples), TRAIN_BATCH_SIZE):
            ...
            i_idx = torch.tensor([t[1] for t in batch], dtype=torch.long, device=device)
            y = torch.tensor([t[2] for t in batch], dtype=torch.float32, device=device)
            probs = _predict_live_batch(model, s_idx, i_idx, val_emb_by_id)
            total_loss += criterion(probs, y).item()
            n_batches += 1
...
def train():
    ...
    val_emb_by_id = _precompute_val_item_embeddings(encoder, val_triples, idx2item, device)
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        val_loss_idx = _run_epoch(model, val_triples, criterion, device, train=False)
        val_loss = _run_epoch_val_live(
            model, val_triples, val_emb_by_id, criterion, device
        )

        print(
            f"epoch {epoch}/{N_EPOCHS}  train_loss={train_loss:.4f}  "
            f"val_loss={val_loss:.4f}  val_loss_idx={val_loss_idx:.4f}"
        )

        if val_loss < best_val:
            best_val = val_loss
            best_val_idx = val_loss_idx
        ...
        json.dump(
            {
                ...
                "best_val_loss_idx": best_val_idx,
                "val_metric": "live_encode",
            },
            f,
        )
    vol.commit()
    print(
        f"Saved best checkpoint (val_loss live-encode={best_val:.4f}, "
        f"idx-embed={best_val_idx:.4f}) to {out_path}"
    )
